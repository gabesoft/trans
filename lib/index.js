...
Trans.prototype.inspect = function() {
    util.inspect(this._state_);
    return this;
};
...
Trans.prototype.first = function(n) {
    return this.map(['slice', 0, 1], 'shift', [util.default, null]);
};

Trans.prototype.last = function(n) {
...
Trans.prototype.filter = function() {
    ...
    this._state_ = trav.filter(this._state_, function (obj) {
        ...
        if (sortField.exists) {
            val = trav.walkValue(val, sortField.path, false);
        }
...
Trans.prototype.mapff = function() {
    ...
    if (paths.src.nil && paths.dst.exists) {
        fail('Destination cannot be a field on the source object');
    } else if (paths.root.nil && paths.src.nil && paths.dst.nil) {
        ...
        result = walk(self._state_, function (node) {
            ...
            val = trav.walk(node.parent, srcField.path, false, function (srcNode) {
                var t = self._create_(srcNode.getValue());
                return t.map.apply(t, funs).value();
            });
...
Trans.prototype.mapf = function() {
    ...
    if (field.iter) { funs.unshift(opath.ITER); }
